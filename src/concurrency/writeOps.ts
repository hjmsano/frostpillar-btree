...
import { type AnyMutationResult, assertNeverMutation } from './helpers.js';
...
export const createRemoveByIdEvaluator = <TKey, TValue>(
  ...
  tree: InMemoryBTree<TKey, TValue>,
) => { type: 'removeById'; entryId: EntryId } | null) => {
  return (tree): { type: 'removeById'; entryId: EntryId } | null => {
    return tree.peekById(entryId) !== null
      ? { type: 'removeById', entryId }
      : null;
  };
};
...
export const createUpdateByIdEvaluator = <TKey, TValue>(
  ...
  tree: InMemoryBTree<TKey, TValue>,
) => { type: 'updateById'; entryId: EntryId; value: TValue } | null) => {
  return (
    tree,
  ): { type: 'updateById'; entryId: EntryId; value: TValue } | null => {
    return tree.peekById(entryId) !== null
      ? { type: 'updateById', entryId, value }
      : null;
  };
};

export const createPopFirstEvaluator = <TKey, TValue>(): ((
  tree: InMemoryBTree<TKey, TValue>,
) => { type: 'popFirst' } | null) => {
...
export const createPopLastEvaluator = <TKey, TValue>(): ((
  tree: InMemoryBTree<TKey, TValue>,
) => { type: 'popLast' } | null) => {
...
export const createClearEvaluator = <TKey, TValue>(): ((
  tree: InMemoryBTree<TKey, TValue>,
) => { type: 'clear' }) => {
